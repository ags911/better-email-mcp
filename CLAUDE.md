...
**`CREDENTIAL_SECRET` is required as soon as `PerSubCredStore` is used at all** (see below) — without it, `/authorize` submissions fail outright with `CREDENTIAL_SECRET env required for HTTP multi-user mode` (mcp-core's `PerPluginStore`, used to encrypt the per-user credential blob regardless of backend). This is a hard prerequisite, not optional hardening — set it in Railway *before* deploying an image built after the durable-credential-store change, or every mailbox re-auth will start failing instead of just being non-durable.
...
- **Durable per-user credential storage**: `selectCredStore()` in `transports/http.ts` now defaults to the durable `PerSubCredStore` (previously only used for `MCP_STORAGE_BACKEND=cf-kv`; everything else fell back to the fully ephemeral `InMemoryCredStore`, which loses every submitted mailbox on every container restart — this bit twice in one day before the fix). `PerSubCredStore` resolves to mcp-core's `LocalFsBackend` (`~/.better-email-mcp/subs/<sub>/config.json`, AES-256-GCM encrypted) when not on Cloudflare. **For this to actually survive a Railway redeploy, a Volume must be mounted at `/home/node/.better-email-mcp` — not at `/home/node` itself.** Mounting at `/home/node` was tried once and took the entire server down: it broke mcp-core's own unrelated `~/.config/mcp/locks` directory (created unconditionally at boot, unhandled on failure), crashing the whole process in a restart loop — not just credential durability, which fails gracefully on its own (`ready()`'s "Credential store UNREACHABLE" log never crashes the server). Also fixed a related bug while wiring this in: `PerSubCredStore.ready()`'s startup probe used a Cloudflare-KV-specific magic key (`'__ready'`) that `LocalFsBackend`'s structured path format rejects outright — now backend-aware (`cred-store.ts`).
- The credential relay form's **"Workspace / username" field is not a secret** — it's a plain-text bucket selector (`stableSubEnabled`), not an access-control credential. Anyone who submits the form with the same value lands in the same storage bucket. `MCP_RELAY_PASSWORD` (mcp-core built-in — timing-safe check, signed session cookie, brute-force lockout) is what actually gates who can reach `/authorize` at all — **set it**; don't assume URL obscurity is the only option, that was wrong the first time this got assessed. It does not gate `/webhooks/resend`, which is protected separately by its own Svix signature check.
